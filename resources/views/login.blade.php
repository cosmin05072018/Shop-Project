...
<img src="{{ asset('photos/backgroundImage.jpg') }}" alt="">
    ...
    <div class="container min-vh-100 d-flex justify-content-center align-items-center text-white">
        <form action="{{ route('validateLogin') }}" method="POST" class="p-3 rounded form-group row col-4">
            @csrf
            ...
            <div class="mb-3">
                ...
                <div class="d-flex align-items-center">
                    ...
                    <input type="text" class="form-control {{ old('email') && !$errors->has('email') ? 'is-valid' : ($errors->has('email') ? 'is-invalid' : '') }}" name="email" value="{{ old('email') }}">
                </div>
                @error('email')
            ...
            <div class="mb-3">
                ...
                <div class="d-flex align-items-center">
                    ...
                    <input type="password" class="form-control {{ old('password') && !$errors->has('password') ? 'is-valid' : ($errors->has('password') ? 'is-invalid' : '') }}" id="password" name="password">
                </div>
                @error('password')
                    <div class="mt-1 w-100 p-1 text-center">{{ $message }}</div>
                @enderror
