...
@section('content')
    <div class="container pt-4">
        <button class="optionsBtn btn mb-3" type="button" data-bs-toggle="offcanvas" data-bs-target="#staticBackdrop" aria-controls="staticBackdrop">
            Options
        </button>
                    ...
                    <div class="card-body ">
                        <form action="{{ route('updateProduct', [$productUpdate->id]) }}" method="POST" enctype="multipart/form-data" class="p-5 rounded">
                            ...
                            <div class="form-group row d-flex align-items-center">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" class="form-control {{ old('title') && !$errors->has('title') ? 'is-valid' : ($errors->has('title') ? 'is-invalid' : '') }}" id="Title" value="{{ old('title') ? old('title') : $productUpdate->title }}" name="title" placeholder="Enter Title Food">
                                    @error('title')
                            ...
                            <div class="form-group row mt-5 d-flex align-items-center">
                                <label for="Price" class="col-sm-2 col-form-label">
                                    <b class="fs-3">Price</b>
                                </label>
                                <div class="col-sm-10">
                                    <input type="number" class="form-control {{ old('price') && !$errors->has('price') ? 'is-valid' : ($errors->has('price') ? 'is-invalid' : '') }}" value="{{ old('price') ? old('price') : $productUpdate->price }}" name="price" id="Price" placeholder="Enter Price Food">
                                        @error('price')
                                        <div class="mt-1 w-100 p-1"><b>{{ $message }}</b></div>
                                    @enderror
                                </div>
